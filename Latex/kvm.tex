...
\textbf{huffenc} & 1,000,000 & 15 & 16 & 64 & 49 $\mu s$\tabularnewline
\textbf{lift} & 1,000,000 & 19 & 19 & 61 & 42 $\mu s$\tabularnewline
...
\begin{itemize}
\item
  The \texttt{huffenc} benchmark executed 1,000,000 loops with an
  extremely stable average latency of 16 $\mu s$.
\item
  The maximum latency peaked at 64 $\mu s$, resulting in an absolute jitter
  of 49 $\mu s$.
\end{itemize}
...
\item
  ...
  maintaining an average latency of 19 $\mu s$ matching its minimum latency.
\item
  The worst-case latency was 61 $\mu s$, yielding a tightly bounded jitter of
  42 $\mu s$ on this KVM baseline.
\end{itemize}
...
virtually zero jitter (1 $\mu s$ peak), while medium and heavy tasks
(\texttt{test3}, \texttt{DEBIE}) show the natural variance introduced by
the standard KVM scheduler (up to 8,831 $\mu s$ of jitter for DEBIE). These
metrics will be crucial for quantifying the exact determinism
improvements when CPU pinning and LL-RT kernel isolation techniques are
introduced.
...
\textbf{huffenc} & 1,000,000 & 15 & 15 & 61 & 46 $\mu s$\tabularnewline
\textbf{lift} & 1,000,000 & 19 & 19 & 58 & 39 $\mu s$\tabularnewline
\textbf{matrix1} & 1,000,000 & 0 & 0 & 0 & 0 $\mu s$\tabularnewline
...
\item
  ...
  of 15 $\mu s$ and a maximum peak of 61 $\mu s$.
\item
  This results in a relatively tight absolute jitter of 46 $\mu s$.
\end{itemize}
...
\item
  ...
  light noise, averaging 19 $\mu s$ with a worst-case execution time of 58
  $\mu s$.
\item
  The resulting jitter is closely bounded at 39 $\mu s$, showing good
...
\item
  ...
  flawlessly under the small noise configuration, logging
  sub-microsecond values across minimum, average, and maximum execution
  times.
\item
  This yields a theoretical absolute jitter of under 1 $\mu s$, indicating
  zero cache thrashing or scheduling interruptions occurred during its
  1,000,000 loops.
\end{itemize}
...
Notably, \texttt{matrix1} shows perfect execution without any measurable
jitter, and \texttt{test3} maintains tight WCET margins. However, the
\texttt{DEBIE} workload still suffers from over 7 milliseconds of
jitter, confirming that unisolated schedulers struggle to guarantee
execution determinism for long-running, parallel tasks even when
...
\textbf{huffenc} & 1,000,000 & 15 & 26 & 69 & 54 $\mu s$\tabularnewline
\textbf{lift} & 1,000,000 & 19 & 19 & 280 & 261 $\mu s$\tabularnewline
\textbf{matrix1} & 1,000,000 & 0 & 0 & 6 & 6 $\mu s$\tabularnewline
...
\item
  ...
  at 15 $\mu s$, the average latency rises to 26 $\mu s$ under noise.
\item
  The maximum latency peaks at 69 $\mu s$, resulting in a relatively
  contained absolute jitter of 54 $\mu s$ for this specific sequential
  pattern.
\end{itemize}
...
\item
  ...
  determinism, with the maximum latency spiking dramatically to 280 $\mu s$.
\item
  This generates an absolute jitter of 261 $\mu s$, indicating that even
  highly repetitive, lightweight control tasks suffer from severe,
  albeit occasional, micro-interruptions under heavy KVM load.
\end{itemize}
...
\begin{itemize}
\item
  The \texttt{matrix1} execution remains extremely fast, maintaing a
  sub-microsecond average latency.
\item
  However, the maximum latency increases to 6 $\mu s$, representing a 6 $\mu s$
  jitter. While numerically small, this indicates minor cache thrashing
  or context switching interference affecting the otherwise
...
unisolated hypervisor scheduling. While some workloads (\texttt{test3},
\texttt{matrix1}) show mild resilience, the heavy \texttt{DEBIE}
workload suffers massive jitter expansion (19,076 $\mu s$), and the critical
\texttt{lift} control loop experiences severe latency spikes (280 $\mu s$
peak). These results suggest the lack of boundaries required for
real-time applications.
...
\textbf{huffenc} & 1,000,000 & 15 & 16 & 62 & 47 $\mu s$\tabularnewline
\textbf{lift} & 1,000,000 & 19 & 20 & 67 & 48 $\mu s$\tabularnewline
\textbf{matrix1} & 1,000,000 & 0 & 0 & 1 & 1 $\mu s$\tabularnewline
...
\item
  CPU pinning effectively bounds the absolute jitter at 47 $\mu s$, restoring
  a high degree of stability to this sequential loop.
\end{itemize}
...
\item
  ...
  strict determinism, maintaining an average latency of 20 $\mu s$.
\item
  The maximum latency reaches only 67 $\mu s$, yielding a tight absolute
  jitter of 48 $\mu s$, which completely eliminates the massive 280 $\mu s$ spike
  seen in the unpinned noise test.
\end{itemize}
...
\item
  ...
  migrations have been successfully mitigated.
\end{itemize}
...
return to near-baseline determinism, and heavier workloads
...
\textbf{huffenc} & 64 & 61 & 69 & 62\tabularnewline
\textbf{lift} & 61 & 58 & 280 & 67\tabularnewline
\textbf{matrix1} & 1 & 0 & 6 & 1\tabularnewline
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 27214.36 ± 1303.61 $\mu s$ & 28845.81 ± 3813.54 $\mu s$\tabularnewline
\textbf{WCET (Max)} & 32409 $\mu s$ & 43849 $\mu s$\tabularnewline
\bottomrule
\end{tabularx}
\end{table*}
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 16.02 ± 3.39 $\mu s$ & 26.21 ± 5.01 $\mu s$\tabularnewline
\textbf{WCET (Max)} & 64 $\mu s$ & 69 $\mu s$\tabularnewline
\bottomrule
\end{tabularx}
\end{table*}

\textbf{PERCENTAGE INCREMENTS (Stress vs.~Baseline):} * Average
Increment: +63.63\% * WCET Increment: +7.81\%
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 19.44 ± 3.00 $\mu s$ & 19.42 ± 3.00 $\mu s$\tabularnewline
\textbf{WCET (Max)} & 61 $\mu s$ & 280 $\mu s$\tabularnewline
\bottomrule
\end{tabularx}
\end{table*}

\textbf{PERCENTAGE INCREMENTS (Stress vs.~Baseline):} * Average
Increment: -0.07\% * WCET Increment: +359.02\%
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 0.00 ± 0.00 $\mu s$ & 0.00 ± 0.01 $\mu s$\tabularnewline
\textbf{WCET (Max)} & 1 $\mu s$ & 6 $\mu s$\tabularnewline
\bottomrule
\end{tabularx}
\end{table*}

\textbf{PERCENTAGE INCREMENTS (Stress vs.~Baseline):} * Average
Increment: +600.00\% * WCET Increment: +500.00\%
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 8363.95 ± 31.59 $\mu s$ & 8457.93 ± 52.13 $\mu s$\tabularnewline
\textbf{WCET (Max)} & 9471 $\mu s$ & 9458 $\mu s$\tabularnewline
\bottomrule
\end{tabularx}
\end{table*}
