...
\midrule
\textbf{Mean ± SD} & 32.87 ± 15.17 $\mu s$ & 35.52 ± 15.50 $\mu s$\tabularnewline
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 33.20 ± 15.12 $\mu s$ & 35.86 ± 15.54 $\mu s$\tabularnewline
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 32.71 ± 15.17 $\mu s$ & 35.58 ± 15.55 $\mu s$\tabularnewline
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 32.89 ± 15.14 $\mu s$ & 35.66 ± 15.60 $\mu s$\tabularnewline
...
\midrule
\textbf{Mean ± SD} & 32.65 ± 15.17 $\mu s$ & 32.76 ± 15.10 $\mu s$\tabularnewline
...
METRIC & vCPU PINNING BASELINE & vCPU PINNING STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 32.59 ± 15.16 $\mu s$ & 33.23 ± 15.09 $\mu s$\tabularnewline
...
METRIC & vCPU PINNING BASELINE & vCPU PINNING STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 32.49 ± 15.19 $\mu s$ & 32.83 ± 15.06 $\mu s$\tabularnewline
...
METRIC & vCPU PINNING BASELINE & vCPU PINNING STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 33.09 ± 15.18 $\mu s$ & 32.73 ± 15.09 $\mu s$\tabularnewline
...
\textbf{huffenc} & 1,000,000 & 15 & 16 & 35 & 20 $\mu s$\tabularnewline
\textbf{lift} & 1,000,000 & 19 & 19 & 85 & 66 $\mu s$\tabularnewline
\textbf{matrix1} & 1,000,000 & 0 & 0 & 9 & 9 $\mu s$\tabularnewline
...
\item
  ...
  1,000,000 times.
\item
  It shows excellent stability with an average latency of 16 $\mu s$ and a
  worst-case peak of 35 $\mu s$.
\item
  The absolute jitter is strictly bounded at 20 $\mu s$, derived from a
...
\begin{itemize}
\item
  The \texttt{lift} benchmark exhibits highly deterministic behavior on
  this baseline, with the average latency (19 $\mu s$) sitting exactly on the
  minimum latency (19 $\mu s$) across one million loops.
\item
  The maximum latency peak of 85 $\mu s$ represents a rare outlier.
\item
  The vast majority of executions cluster tightly at 19-20 $\mu s$, showing
  that the unisolated baseline scheduler handles this specific workload
...
lightweight tasks (\texttt{matrix1}, \texttt{lift}, \texttt{huffenc})
execute with near-perfect determinism, while heavier tasks
(\texttt{DEBIE}) suffer from severe scheduling jitter. The outliers
observed in \texttt{lift} (85 $\mu s$) and \texttt{huffenc} (35 $\mu s$) are the
specific OS noise artifacts that isolation mechanisms aim to eliminate.
When transferring these workloads to virtualized setups, tracking the
...
\textbf{huffenc} & 1,000,000 & 15 & 16 & 35 & 20 $\mu s$\tabularnewline
\textbf{lift} & 1,000,000 & 19 & 19 & 36 & 17 $\mu s$\tabularnewline
\textbf{matrix1} & 1,000,000 & 0 & 0 & 8 & 8 $\mu s$\tabularnewline
...
\begin{itemize}
\item
  The \texttt{huffenc} benchmark shows excellent stability across
  1,000,000 iterations.
\item
  With an average latency of 16 $\mu s$ and a worst-case peak of 35 $\mu s$, the
  absolute jitter remains tightly bounded at 20 $\mu s$.
\end{itemize}
...
\item
  ...
  exhibits highly deterministic behavior, maintaining an average latency
  of 19 $\mu s$.
\item
  The maximum latency peak is only 36 $\mu s$, representing a noticeably
  tighter jitter (17 $\mu s$) compared to the unisolated baseline.
\end{itemize}
...
\item
  The absolute worst-case execution time is capped at an exceptionally
  low 8 $\mu s$.
\end{itemize}
...
repetitive tasks (\texttt{matrix1}, \texttt{lift}, \texttt{huffenc}) can
maintain extreme determinism, with jitter boundaries narrowing
significantly (such as \texttt{lift} dropping to a 36 $\mu s$ max peak).
Heavier workloads like \texttt{DEBIE} continue to exhibit substantial
variance.
...
\textbf{huffenc} & 1,000,000 & 15 & 19 & 182 & 167 $\mu s$\tabularnewline
\textbf{lift} & 1,000,000 & 17 & 20 & 142 & 125 $\mu s$\tabularnewline
\textbf{matrix1} & 1,000,000 & 0 & 0 & 45 & 45 $\mu s$\tabularnewline
...
\begin{itemize}
\item
  While the \texttt{huffenc} task maintains a relatively stable average
  latency of 19 $\mu s$, the maximum latency spikes dramatically to 182 $\mu s$.
\item
  This results in an absolute jitter of 167 $\mu s$, showing that even
...
\begin{itemize}
\item
  The \texttt{lift} benchmark's determinism breaks down under the big
  noise configuration, expanding to a maximum latency of 142 $\mu s$.
\item
  Compared to previous baselines, the average latency rises slightly to
  20 $\mu s$, but the 125 $\mu s$ absolute jitter indicates substantial scheduling
  delays.
\end{itemize}
...
\texttt{matrix1} and \texttt{lift} experience significant latency
spikes, while heavy tasks like \texttt{DEBIE} become entirely
...
\textbf{huffenc} & 1,000,000 & 15 & 16 & 42 & 27 $\mu s$\tabularnewline
\textbf{lift} & 1,000,000 & 19 & 19 & 43 & 24 $\mu s$\tabularnewline
...
\item
  ...
  and a peak maximum latency of 42 $\mu s$.
\item
  CPU pinning effectively bounds the absolute jitter at 27 $\mu s$, restoring
  a high degree of stability to this lightweight loop.
\end{itemize}
...
\item
  ...
  strict determinism, maintaining an average latency of 19 $\mu s$.
\item
  The maximum latency reaches only 43 $\mu s$, yielding a tight absolute
  jitter of 24 $\mu s$.
\end{itemize}
...
catastrophic latency spikes observed in the unpinned tests. Lightweight
tasks (\texttt{matrix1}, \texttt{lift}, \texttt{huffenc}) return to
near-baseline determinism, and heavier workloads (\texttt{DEBIE},
\texttt{test3}) see their jitter margins compressed significantly.
...
\textbf{huffenc} & 35 & 35 & 182 & 42\tabularnewline
\textbf{lift} & 85 & 36 & 142 & 43\tabularnewline
\textbf{matrix1} & 9 & 8 & 45 & 11\tabularnewline
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 27175.71 ± 1309.25 $\mu s$ & 28196.64 ± 3277.83 $\mu s$\tabularnewline
\textbf{WCET (Max)} & 31048 $\mu s$ & 57314 $\mu s$\tabularnewline
\bottomrule
\end{tabularx}
\end{table*}
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 16.18 ± 1.22 $\mu s$ & 19.05 ± 5.71 $\mu s$\tabularnewline
\textbf{WCET (Max)} & 35 $\mu s$ & 182 $\mu s$\tabularnewline
\bottomrule
\end{tabularx}
\end{table*}
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 19.23 ± 1.32 $\mu s$ & 20.43 ± 4.91 $\mu s$\tabularnewline
\textbf{WCET (Max)} & 85 $\mu s$ & 142 $\mu s$\tabularnewline
\bottomrule
\end{tabularx}
\end{table*}
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 0.00 ± 0.17 $\mu s$ & 0.01 ± 0.30 $\mu s$\tabularnewline
\textbf{WCET (Max)} & 9 $\mu s$ & 45 $\mu s$\tabularnewline
\bottomrule
\end{tabularx}
\end{table*}
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 8328.16 ± 38.48 $\mu s$ & 9182.95 ± 1317.37 $\mu s$\tabularnewline
\textbf{WCET (Max)} & 8414 $\mu s$ & 14786 $\mu s$\tabularnewline
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{tabularx}{\linewidth}{@{}YYYYYYY@{}}
\toprule
\textbf{Benchmark} & \textbf{Config} & \textbf{Min ($\mu s$)} & \textbf{Media ($\mu s$)} & \textbf{p99 ($\mu s$)} & \textbf{Massimo ($\mu s$)} & \textbf{Overflow}\tabularnewline
\midrule
...
\toprule
\textbf{Benchmark} & \textbf{$\delta Media$} & \textbf{$\delta p99$} & \textbf{$\delta Massimo$}\tabularnewline
\midrule
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 37.17 ± 14.69 $\mu s$ & 40.20 ± 16.22 $\mu s$\tabularnewline
...
METRIC & BASELINE & STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 32.40 ± 15.36 $\mu s$ & 35.58 ± 15.50 $\mu s$\tabularnewline
...
METRIC & vCPU PINNING BASELINE & vCPU PINNING STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 40.00 ± 14.71 $\mu s$ & 39.15 ± 15.23 $\mu s$\tabularnewline
...
METRIC & vCPU PINNING BASELINE & vCPU PINNING STRESS WORKLOAD\tabularnewline
\midrule
\textbf{Mean ± SD} & 33.16 ± 15.12 $\mu s$ & 33.20 ± 15.09 $\mu s$\tabularnewline
